...
cat_type = 'flagship'               # can be 'flagship', 'sdhod', 'pinocchio' or 'box'
shuffled_fluxes=False
...
# SELECTION SECTION
selection_data_tag = None # 'MWext'
selection_random_tag = None #'MWext'
...
def selection_data_fname(sel_tag=selection_data_tag):
    if sel_tag is not None:
        return build_fname('Selections',['data',sel_tag,galcat_kernel(),sh_tag()])
    else:
        return ''
...
def dndz_fname(sel_tag=selection_data_tag):
    return build_fname('NumberCounts',['dndz',sel_tag,galcat_kernel(),sh_tag(),sm_tag(),rsd_tag()])
def random_fname():
...
def numbercounts_fname( z1, z2, sel_tag=selection_data_tag): 
    return build_fname('NumberCounts',['numbercounts',sel_tag,galcat_kernel(),sh_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)])
def cls_fname( z1, z2):
    return build_fname('Cls',['Cls',selection_tag(),galcat_kernel(),sh_tag(),sm_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)])

def LE3_data_fname( z1, z2):
    return build_fname('Catalogs4LE3',['data',selection_data_tag,galcat_kernel(),sh_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)])
...
def delta_data_fname( z1, z2):
    return build_fname('Cls',['delta_data',selection_data_tag,galcat_kernel(),sh_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)])
...
def params_fname( z1, z2): 
    return build_fname('PkParams',['params',selection_tag(),galcat_kernel(),sh_tag(),random_sm_tag(),rsd_tag(),alpha_tag(),ngrid_tag(),Lbox_tag(),'z{}-{}'.format(z1,z2)],ext='.ini')
def pk_fname( z1, z2): 
    return build_fname('Pks',['pk',selection_tag(),galcat_kernel(),sh_tag(),random_sm_tag(),rsd_tag(),alpha_tag(),ngrid_tag(),Lbox_tag(),'z{}-{}'.format(z1,z2)],ext='')
def script_fname( num): 
    return build_fname('PkScripts',['script',selection_tag(),galcat_kernel(),sh_tag(),random_sm_tag(),rsd_tag(),alpha_tag(),ngrid_tag(),Lbox_tag(),'{}'.format(num)],ext='.sh')

def plot_dndz_fname(sel_tag=selection_data_tag):
    return build_fname('Plots',['dndz',sel_tag,galcat_kernel(),sh_tag(),sm_tag(),rsd_tag()],ext='.png')
def plot_numbercounts_fname( z1, z2, sel_tag=selection_data_tag):
    return build_fname('Plots',['numbercounts',sel_tag,galcat_kernel(),sh_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)],ext='.png')
def plot_cls_fname( z1, z2): 
    return build_fname('Plots',['cls',selection_tag(),galcat_kernel(),sh_tag(),sm_tag(),rsd_tag(),'z{}-{}'.format(z1,z2)],ext='.png')
def plot_pk_fname( z1, z2): 
    return build_fname('Plots',['pk',selection_tag(),galcat_kernel(),sh_tag(),random_sm_tag(),rsd_tag(),alpha_tag(),ngrid_tag(),Lbox_tag(),'z{}-{}'.format(z1,z2)],ext='.png')
